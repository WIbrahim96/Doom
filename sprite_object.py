...
class AnimatedSprite(SpriteObject):
    def __init__(self, game, path='resources/sprites/animated_sprites/green_light/0.png',
        ...
        self.images = self.get_images(self.path)  # Get images from folder
    ...
    def get_images(self, path):  
        images = deque()
        try:
            # 🔹 Ensure files are sorted properly, handling both numeric and non-numeric filenames
            files = sorted(os.listdir(path), key=lambda f: int(f.split('.')[0]) if f.split('.')[0].isdigit() else float('inf'))
    ...
    def animate(self, image_list=None):  
